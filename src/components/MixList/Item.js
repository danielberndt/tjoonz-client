...
import {extractFirstElement, extractArtworkSrc, extractTermsByTaxonomy} from "../../utils/extract";
import {toPublishDate, toTermNames, toTermSpanLabels} from "../../utils/format";
    ...
    return (
      <div className="mix-list-item" onClick={() => this.props.onClick(this.props.slug)}>
        ...
        <div className="controls">
          <button onClick={() => this.props.onPlay(this.props.id)}>play</button>
          <button />
        </div>
        <div className="artists">
          <span dangerouslySetInnerHTML={{__html: toTermNames(artists)}} />
        </div>
        <div className="title">
          <span dangerouslySetInnerHTML={{__html: this.props.title.rendered}} />
        </div>
