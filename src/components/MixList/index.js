...
export default class extends Component {
  ...
  scrolled = () => {
    ...
    if (
      ...
      this.props.mixes.length &&
      !this.props.isLoading &&
      !this.props.isExhausted
    ) {
  ...
  footer = () => {
    if (this.props.isExhausted) {
      return (
        <div className="mix-list-footer">Still need more? Try relaxing the filter criteria.</div>
      );
    } else {
      return (
        <button className="mix-list-footer" onClick={this.props.onScrollToBottom}>
          <span>Load more mixes</span>
        </button>
      );
    }
  };
