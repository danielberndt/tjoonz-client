...
export default class extends Component {
  ...
  loadAvailableFilters = async (endpoint, top = 10) => {
    ...
    return fetchPage(endpoint, 1, otherOptions, true).then(otherResults =>
      this.setState({
        filters: Object.assign({}, this.state.filters, {
          [endpoint]: [...topResults, ...otherResults],
        }),
      })
    );
  };
  ...
  renderActiveFilters = (key, activeIds) => {
    if (!this.state.loading) {
      ...
      return activeIds.map(item => (
        <div key={`${key}-${item}`} className="filter-item loading">
          &hellip;
        </div>
      ));
    }
  ...
  expandButton = () => {
    if (this.state.loading) {
      ...
      if (this.state.genresHidden) {
        return (
          <button className="expand" onClick={() => this.setState({genresHidden: false})}>
            <span>Show all genres</span>
          </button>
        );
      } else {
        return (
          <button className="expand" onClick={() => this.setState({genresHidden: true})}>
            <span>Show less genres</span>
          </button>
        );
      }
    }
            ...
            : this.state.filters.genre
                ...
                .map(genre => (
                  <Item
                    ...
                    count={genre.count}
                    active={
                      Boolean(this.props.filter.genre) &&
                      Boolean(this.props.filter.genre.ids.indexOf(genre.id) !== -1)
                    }
                    onToggle={this.filterChanged}
                  />
                ))}
